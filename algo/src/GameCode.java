...
public class Main {
	...
	static int[] preMyval = new int[10];//내가선택한 2번째 이전 값 상대방이 내값 선택에 대한 대응을 알기위함
	...
	static Opponent Me = new Opponent() { 
		public int hunt(int opp, int turn, int opp_prev, int opp_last_pattern[][]) {
			...
			if (turn>=2) {//내가 선택한 값에 대한 상대의 선택을 확인가능한 턴이 3턴째 부터
				if (preMyval[turn-2]==0) {//내가사슴 낸거보고
					if (opp_prev==0) {//이사람이 사슴냈다?
						if (preMyval[turn-1]==0) {//내가 이전에 사슴냈으면 뱀
							returnVal = 2;
							preMyval[turn] = returnVal;
							return returnVal;
						}else {
							returnVal = 0;
							preMyval[turn] = returnVal;
				...
				}else if(preMyval[turn-2]==1) {//내가 토끼 낸거보고
					if (opp_prev==0) {
						returnVal = 0;
						preMyval[turn] = returnVal;
						return returnVal;
				...
				}else if(preMyval[turn-2]==2) {//내가 뱀 낸거보고
					if (opp_prev==1) {//토끼냈다? 
						returnVal = 0;
						preMyval[turn] = returnVal;
						return returnVal;
					}else if(opp_prev==2) {//뱀냈다? 바로혼내줌
						returnVal = 2;
						preMyval[turn] = returnVal;
						return returnVal;
					}else {
				...
				if (turn==0&&opp_last_pattern[0][9]==1) {
					returnVal = 2;
					preMyval[turn] = returnVal;
					return returnVal;
				}else if(turn==0&&opp_last_pattern[0][9]!=1) {
					returnVal = 0;
					preMyval[turn] = returnVal;
					return returnVal;
				}
			...
			preMyval[turn] = returnVal;
			return returnVal; // 반드시 0 또는 1 또는 2로 리턴해야합니다.
		}
	};
	...
	static Opponent Opponent3 = new Opponent() { 
	  ...
	  private static int[] gumi4_3_log = new int[10]; // n-1 턴의 상대방의 선택
	    private static int[] gumi4_3_trust = new int[150];   // 상대방에 대한 내 신뢰도
	    
	    static Opponent Opponent4 = new Opponent() { 
	        public int hunt(int opp, int turn, int opp_prev, int opp_last_pattern[][]) {
	            gumi4_3_log[turn] = opp_prev;

	            // 첫 턴은 무조건 사슴으로 시작한다. (상대방의 신뢰를 얻기 위함)
	            if (turn == 0){
	                gumi4_3_trust[opp] = -opp_last_pattern[0][0]; // 상대방이 정보가 없을 때 맨 처음 고른 선택지 (상대방의 기본 성향이라 간주)
	                return 0;
	            }

	            // 한 번 더 눈치를 본다.
	            if (turn == 1){
	                gumi4_3_trust[opp] = Math.max(gumi4_3_trust[opp], opp_prev);
	                return 0;
	            }

	            // 항상 첫 턴에는 무조건 사슴을 제안했는데도 0번째 턴을 제외하고 토끼나 뱀을 골라준다면 뱀과 사슴만 선택한다.
	            if (opp_prev != 0){
	                gumi4_3_trust[opp] = -2;
	            }

	            if (gumi4_3_trust[opp] > 0){
	                return 0;
	            }else{
	                return turn%4 == 0? 0:2;
	            }
